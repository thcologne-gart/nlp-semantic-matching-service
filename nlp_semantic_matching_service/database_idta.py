...
def get_values_sec(
    ...
    for element in collection_values:
        sec_id_short_path_value = id_short_path_value
        ...
        if se_type == "SubmodelElementCollection":
            ...
            sec_id_short_path_value.append(i)
            #print(sec_id_short_path_value)
            new_row = pd.DataFrame(
                {
                    "SubmodelContent": submodel_str,
                    "AASId": aas_id,
                    "AASIdShort": aas_name,
                    ...
                    "IdShortPathValues": str(sec_id_short_path_value)
                }
            )
            print('-------------------')
            print(new_row.loc[0, 'IdShortPathValues'])
            print(new_row.loc[0, 'SEIdShort'])
            df = pd.concat([df, new_row], ignore_index=True)

            content = []
            content.append(element)
            #sec_id_short_path_value.pop()
...
def read_aas(data, aas, submodels, assets, conceptDescriptions, submodels_ids, metalabel):
    ...
    for submodel in submodels:
        submodel_str = json.dumps(submodel)
                ...
                new_row = pd.DataFrame(
                    {
                        ...
                        "IdShortPathValues": str(id_short_path_value)
                    }
                )
                print('-------------------')
                print(new_row.loc[0, 'IdShortPathValues'])
...
def post_aas_basyx(aas_file, data, aas_id, aas_name):
    ...
    for submodel in submodels:
        ...
        print(response_submodel)             

    return response


# Mit ECLASS def index_corpus(aas_file, data, model, metalabel, client_chroma, client_chroma_eclass):
def index_idta_submodels(aas_file, data, model, metalabel, client_chroma):
    ...
    print(aas_df[['SEIdShort', "IdShortPathValues"]])
    # aas_df_embeddings = encode(aas_df, model)
    aas_df = encode(aas_df, model)
    
    # Mit ECLASS aas_df = predict_aas_type(aas_df, model, client_chroma_eclass)
